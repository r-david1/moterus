...
// (mismo criterio que identidad/adaptadores/confianza y
// acceso/adaptadores/confianza: este test verifica la traducción del ACL,
// no el caso de uso de Confianza).
...
func origenConHuellaDePrueba(t *testing.T, ip, huella string) tenenciadominio.OrigenSolicitud {
	t.Helper()
	origen, err := tenenciadominio.NuevoOrigenSolicitud(ip, "agente-de-prueba", huella, "id-solicitud-de-prueba")
	if err != nil {
...
// HuellaDispositivo, IDUsuario e IDSolicitud: tenencia/puertos.
// ResultadoIntento ya llevaba Origen (huella, id de solicitud) e IDUsuario,
// y hasta esta extensión el ACL los descartaba al traducir.
...
// identidad/adaptadores/confianza y acceso/adaptadores/confianza.
func TestEvaluadorConfianzaReal_NuncaExponeCamposDeRiesgo(t *testing.T) {
	...
	ast.Inspect(f, func(n ast.Node) bool {
		...
		if prohibidos[ident.Name] {
			...
			t.Errorf("%s:%d menciona %q: INV-RIES-09 prohíbe que PuntajeRiesgo/NivelRiesgo/SenalesDeRiesgo crucen la frontera de contexto hacia tenencia/puertos.DecisionConfianza", archivo, pos.Line, ident.Name)
		}
		return true
	})
}
