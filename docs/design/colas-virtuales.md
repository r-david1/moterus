...
11. **Prueba de carga con `k6`** (lo pide la ficha del agente y es la única forma de validar la premisa entera): 5 000 usuarios ingresando en 10 s contra una sala de `ritmo=50/s`, verificando (a) que el endpoint real nunca supera 50 req/s admitidas, (b) que la cola drena en ~100 s sin errores 5xx del sistema protegido, y (c) que el p99 del endpoint de ingreso se mantiene en milisegundos mientras eso ocurre. **Implementado**: `test/carga/colas_virtuales_test.js` + `abrir_sala_carga.sql`/`cerrar_sala_carga.sql` (abren la sala de alcance sistema directo en Postgres — ADR 0045, no hay endpoint HTTP para eso) + `analizar_admision.sh` (post-procesa la salida `--out json=` de k6 para el criterio (a), que k6 no puede expresar como threshold nativo). Verificado de punta a punta con curl manual y con una corrida automatizada de k6 a escala reducida (500 usuarios, mismo ritmo=50/s): 100% de los checks en verde, p99 del endpoint de ingreso de ~2-3ms. La escala completa (5000/~100s) no se pudo sostener dentro del entorno de desarrollo interactivo donde se escribió esto (el proceso del servidor era terminado por el propio entorno bajo carga sostenida de más de ~100s, no por un fallo de la aplicación) — el script queda a la escala que pide este documento para correrse en CI/una VM real. El proceso de verificación encontró y corrigió tres problemas reales antes de poder confiar en el resultado (rate limiting de `ingreso_a_sala` por una sola IP, una dependencia de red externa del endpoint protegido elegido primero, y un `reloj_desde` que dejaba preadmitir posiciones fantasma durante la espera al reconciliador) — el detalle completo está en el mensaje del commit que lo introdujo.
