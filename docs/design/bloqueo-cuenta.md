...
2. **`TURNSTILE_SECRET_KEY` es requisito duro de arranque en producción — implementado.** Antes, `APP_ENV=production` sin esa llave arrancaba con un `slog.Error` y el captcha fail-closed — es decir, arrancaba con **la única salida del cooldown de cuenta cerrada**, la violación de INV-BLQ-04. Se pidió conformidad operativa explícita al usuario del proyecto (única pieza de este documento con consecuencia operativa: un despliegue existente sin la llave deja de arrancar), la dio el 2026-09-14 ("todo despliegue de producción va a tener Turnstile configurado"), y se implementó en `cmd/api/main.go` (`construirEvaluadorDeRiesgo`, `log.Fatalf` si `cfg.EntornoApp == "production" && cfg.TurnstileSecretKey == ""`), siguiendo el patrón ya existente de `exigirEnProduccionOAdvertir` (ADR 0020: `ACCESO_LLAVE_FIRMA`/`ACCESO_EMISOR`/`ACCESO_AUDIENCIA`), no el de ADR 0017 (`DATABASE_URL_APLICACION` solo emite un `WARN` y sigue arrancando — la mención original de este documento a ese precedente estaba equivocada, corregida acá). Verificado en vivo: `APP_ENV=production` sin la llave termina el proceso con `exit status 1` y el mensaje de arriba; con la llave configurada, arranca completo y sirve tráfico (`auth-service escuchando en :8080 (env=production)`).
...
2. ~~Sin `TURNSTILE_SECRET_KEY` en producción, el único mecanismo de contención por cuenta se queda sin salida (INV-BLQ-04 violada por configuración).~~ **Cerrado el 2026-09-14**: con conformidad explícita del usuario del proyecto, `cmd/api/main.go` falla al arrancar en `APP_ENV=production` sin esa llave (ver §3.2 punto 2 y §11 paso 4b). Ya no es un riesgo residual, se deja el punto para no perder el rastro de que existió.
...
4. **ADR 0053 + la columna nueva en la tabla de asimetrías** (`internal/confianza/README.md`) — la regla de la salida alcanzable, documental y barata.
   - **Paso 4b — fallar al arrancar en producción sin `TURNSTILE_SECRET_KEY`: implementado.** Era el único cambio de todo el documento con consecuencia operativa (un despliegue existente sin la llave deja de arrancar). El usuario del proyecto dio conformidad explícita el 2026-09-14 ("todo despliegue de producción va a tener Turnstile configurado"); se implementó en `cmd/api/main.go` (`construirEvaluadorDeRiesgo`), siguiendo el patrón ya existente de `exigirEnProduccionOAdvertir` (ADR 0020), y se verificó en vivo: sin la llave el proceso termina con `exit status 1` antes de escuchar ningún puerto; con la llave, arranca completo.
