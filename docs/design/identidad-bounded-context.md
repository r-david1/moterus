...
> **Actualización post-implementación (cierre del hito de infraestructura real, ver sección 8):** de esta tabla, **0008** y **0009** ya están resueltos — ambos se implementaron y se verificaron contra Postgres/HTTP reales sin desviarse de lo aquí propuesto. El resto (0010–0016) sigue siendo candidato/backlog: no bloquearon el cierre del MVP porque `UnidadDeTrabajo` (0010), UUIDv7 (0012) y "comandos con primitivos" (0016) ya se implementaron *de facto* como parte del trabajo de dominio/aplicación/infraestructura sin que hiciera falta separarlos en un ADR propio (no fueron decisiones disputadas durante la implementación), y 0011, 0013, 0014, 0015 todavía no tienen código que los ejerza (bloqueo de cuenta, endpoint de verificación de correo, etc. — ver sección 8). Se dejan numerados tal como estaban para no romper referencias ya usadas en comentarios de código (p. ej. `errores_http.go` referencia "ADR candidato 0013").
>
