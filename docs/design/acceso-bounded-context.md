...
| **0020** | Algoritmo de firma (**EdDSA/Ed25519** propuesto, RS256 como escape) y gestión/rotación de llaves | Candidato | HMAC queda descartado sin discusión por INV-ACC-13 (un secreto compartido convierte a cualquier verificador en emisor, incompatible con el alcance multi-producto de ADR 0002). Entre EdDSA y RS256: Ed25519 da firmas de 64 bytes contra ~256, verificación más rápida y cero parámetros que configurar mal; RS256 tiene soporte universal en clientes no-Go. Como el JWKS puede servir ambos tipos a la vez, cambiar de algoritmo es un evento de rotación de llaves, no una ruptura. **Fase 1 propuesta**: llaves en configuración (env), `kid` = thumbprint RFC 7638, rotación por despliegue manteniendo la llave anterior en el conjunto de verificación durante ≥ `vidaTokenAcceso`. Rotación con estado en base de datos = backlog. |
| **0021** | Transporte del token de refresco: cookie `HttpOnly`+`Secure`+`SameSite=Strict` con `Path` acotado, o cuerpo JSON | Candidato | La cookie protege del robo por XSS (que es el vector realista contra una SPA) y el `Path` acotado limita su envío a un solo endpoint; el cuerpo JSON es imprescindible para clientes móviles/servidor. Propuesta: ambos, con cookie por defecto y modo bearer explícito. Requiere decidir también la defensa CSRF del endpoint de renovación (`SameSite=Strict` + verificación de `Origin`). |
...
| **0025** | El token de acceso no lleva claims de autorización ni PII | Candidato | Un `roles: [...]` dentro del JWT es autorización cacheada durante 10 minutos: quitarle un permiso a alguien tardaría hasta ese tiempo en surtir efecto, y el claim quedaría desincronizado con Tenencia sin que nadie lo note. Además Tenencia no existe. Decisión adyacente: cuando exista, ¿los roles van al token o se consultan por puerto en cada request? Eso será un ADR de Tenencia, no de Acceso. |
| **0026** | Alcance de la auditoría de Acceso: no auditar validaciones de token salvo rechazos con valor de señal | Candidato | El advisory lock de ADR 0005 serializa toda la cadena; auditar cada validación pondría el hot path del sistema detrás de un lock global. Es una excepción consciente a la lógica de "auditar todo", con criterio explícito de qué sí entra. |
| **0027** | Duplicación deliberada de `OrigenSolicitud`, `IDUsuario` y `EventoDominio` en `acceso/dominio` en vez de un shared kernel | Candidato | Ver §1.7. Podría parecer un descuido en la revisión de código si no está escrito por qué. |
