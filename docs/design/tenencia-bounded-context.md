...
| **0032** | `Membresia` es **agregado propio**, no una colección dentro de `Organizacion`; el invariante "al menos un propietario" se sostiene con candado de la fila raíz **más** `CONSTRAINT TRIGGER` diferido | Candidato | El repositorio tiene dos precedentes opuestos (`FactorMFA` fuera de `Usuario`, `TokenRefrescoEmitido` dentro de `Sesion`) y sin ADR alguien va a "corregir" esto en una revisión de código. Hay que dejar escrito cuál gana y por qué (§1.2), y sobre todo por qué el invariante entre agregados necesita **dos** mecanismos: el candado da el error de negocio correcto y accionable, el trigger diferido garantiza que ninguna transacción concurrente ni ningún caso de uso futuro lo eluda. |
| **0033** | La invitación es un **agregado propio con token opaco de un solo uso**, redimible solo por un usuario autenticado, activo y con el correo verificado coincidente | Candidato | La alternativa aparentemente más simple —crear la membresía en estado `pendiente`— es **imposible**: exige resolver correo→`usuario_id`, y el único puerto de Identidad que lo permitiría sería un oráculo de enumeración de cuentas operable desde el endpoint de invitación, contra INV-ID-11 y ADR candidato 0013. Además hay que justificar por qué el token no basta por sí solo (INV-TEN-21): un enlace de invitación se reenvía, se filtra en un historial de chat y se indexa; exigir además el control del buzón convierte una fuga del enlace en un no-evento. |
