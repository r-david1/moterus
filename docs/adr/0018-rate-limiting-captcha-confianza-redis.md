...
> **Corrección (ADR 0052)**: esta sección, tal como se escribió originalmente, aplicaba el mismo escalado a la clave de IP y a la de cuenta por igual, sin notar que la clave de cuenta (`confianza:rl:cuenta:<accion>:<correoNormalizado>`) la posee la potencial víctima, no quien la ataca — un tercero sin ninguna credencial podía escalar el TTL de una cuenta ajena hasta el tope de 2h con solo 9 peticiones HTTP. Desde ADR 0052, `Umbral.BackoffMaximo` acota el nivel Cuenta a su propia `Ventana` (nunca escala); el tope de 2h descrito arriba sigue aplicando tal cual, sin cambios, al nivel IP — ahí sí es el atacante quien paga el escalado.
...
- Un despliegue que defina `REDIS_URL` pero no `TURNSTILE_SECRET_KEY` en producción queda con el captcha fail-closed: cualquier cooldown de cuenta se vuelve un bloqueo duro sin posibilidad de bypass humano hasta que se configure la llave. **Esta consecuencia estaba subestimada en la redacción original de este ADR**, que decía "15 minutos" — antes de ADR 0052, el escalado exponencial no distinguía IP de cuenta, así que un tercero sin ninguna credencial podía llevar ese bloqueo hasta 2 horas (el tope `backoffMaximoPorDefecto`) con solo 9 peticiones HTTP contra la cuenta de otro (ver `docs/adr/0052-cooldown-exponencial-solo-en-clave-propia.md` y `docs/design/bloqueo-cuenta.md` §3.1). Desde ADR 0052 el bloqueo sin captcha vuelve a ser, como esta consecuencia siempre asumió, de la ventana nominal por acción (15 minutos en login) — pero sigue sin bypass humano hasta que `TURNSTILE_SECRET_KEY` esté configurada, y esa dependencia dura es ahora una regla explícita (ADR 0053, "regla de la salida alcanzable"), no solo una nota de este ADR. Es información que debe estar en cualquier checklist de "listo para producción" (mismo espíritu que la nota equivalente de ADR 0017 sobre `DATABASE_URL_APLICACION`).
