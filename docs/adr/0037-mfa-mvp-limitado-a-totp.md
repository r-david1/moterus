...
- **Implementar los tres tipos desde el MVP**: descartada en su momento. Email y SMS exigen un proveedor transaccional real. En ese momento el stack no tenía ninguno: `NotificadorCorreoLog` (Identidad) y `NotificadorInvitaciones` (Tenencia) eran stubs *log-only* explícitamente marcados como no aptos para producción. El correo ya se cerró después (ADR 0054/0055: `NotificadorCorreoTransaccional`, directo vía SMTP genérico, sin automatización externa), pero SMS sigue sin ningún proveedor integrado — construir un `EmisorOTP` de SMS sobre una base sin proveedor real seguiría siendo fabricar un canal de segundo factor que no puede probarse de verdad.
